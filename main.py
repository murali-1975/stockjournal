...
Usage:
    ...
    python main.py --refresh-cache   Ignore cached market data and force updates from Yahoo Finance
    python main.py --debug           Enable verbose debugging logs
...
# Configure logger based on CLI flags early
        ...
        while True:
            try:
                ...
                if sys.stdin and sys.stdin.isatty():
                    ...
                    logger.warning(f"   Please save and close the Excel file so the program can {action_word} it, then press Enter to retry...")
                    try:
                        input()
                    ...
                    if attempt % 6 == 1: # Print every 30 seconds
                        logger.warning(f"⚠️  [FILE LOCKED] Waiting for '{os.path.basename(filepath)}' to be released (locked by another process)...")
                    time.sleep(5)


def main(update_only: bool = False, gsheet_target: str = None, refresh_cache: bool = False):
    """
    ...
    logger.debug(f"Checked file accessibility in {time.time() - t0:.2f} seconds.")

    # Step 1: Load configuration
    t0 = time.time()
    config = load_config(config_path)
    logger.info(f"Loaded configuration from input.cfg in {time.time() - t0:.2f} seconds.")

    # Step 2: Load existing master database
    t0 = time.time()
    master_df = load_master_database(output_path)
    logger.info(f"Loaded master database in {time.time() - t0:.2f} seconds.")

    if update_only:
        logger.info("LTP/EMA Update Mode: Skipping new trade ingestion.")
        df = master_df
    else:
        ...
        if new_df is None:
            logger.error(f"Failed to load trade data from '{os.path.basename(input_path)}'.")
            sys.exit(1)

        # Step 4: Merge and deduplicate
        df = merge_and_deduplicate(master_df, new_df)
        logger.info(f"Ingested and merged trades in {time.time() - t0:.2f} seconds.")
    
    if df is None or df.empty:
        logger.warning("No trade data found to process.")
        return

    # Step 4.5: Check for auto-adjustments (Stock Splits/Bonuses)
    if not df.empty:
        t0 = time.time()
        was_modified, df = process_splits(df, base_dir=current_dir, refresh_cache=refresh_cache)
        if was_modified:
            logger.info("Historical trades adjusted in-memory. Will be saved to Excel.")
        logger.info(f"Completed corporate actions check in {time.time() - t0:.2f} seconds.")

    # Step 5: Process grouped trades with Tranche/Cheat labels
    t0 = time.time()
    logger.info("Processing tranches and groupings...")
    grouped_df = process_grouped_trades(df, config)
    logger.info(f"Processed grouped trades in {time.time() - t0:.2f} seconds.")

    # Step 6: Calculate portfolios, PnL, and Stop Loss
    t0 = time.time()
    logger.info("Syncing market data and calculating portfolios...")
    from src.data_io import load_price_updates
    price_updates = load_price_updates(output_path)
    portfolio_df, overall_df = calculate_portfolios(df, grouped_df, config, price_updates, refresh_cache=refresh_cache)
    logger.info(f"Synced market data and calculated portfolios in {time.time() - t0:.2f} seconds.")
    ...
    logger.info(f"Fetched benchmark returns in {time.time() - t0:.2f} seconds.")

    # Step 7: Save everything to Excel
    t0 = time.time()
    logger.info("Writing results to Excel sheets...")
    save_workbook(df, grouped_df, portfolio_df, overall_df, output_path, benchmark_returns)
    logger.info(f"Excel workbook updated in {time.time() - t0:.2f} seconds.")
    ...
    if gsheet_target:
        ...
        logger.info(f"Google Sheets update completed in {time.time() - t0:.2f} seconds.")

    logger.info(f"🚀 Stock Journal pipeline completed successfully in {time.time() - start_time:.2f} seconds.")
...
if __name__ == "__main__":
    ...
    if '--test' in sys.argv:
        ...
        if len(sys.argv) == 2 or (len(sys.argv) == 3 and '--debug' in sys.argv):
    ...
    if '--recommend' in sys.argv:
        ...
        for f in rec_filters:
            if f in valid_options:
                ...
                logger.warning(f"⚠️ Invalid recommendation filter '{f}' ignored. Valid values: all, add, buy, sell.")
    ...
    if '--update' in sys.argv:
        main(update_only=True, refresh_cache=refresh_cache)
    elif '--watch' in sys.argv:
        try:
            idx = sys.argv.index('--watch')
            interval_minutes = float(sys.argv[idx + 1])
        except (ValueError, IndexError):
            logger.error("Usage: python main.py --watch <minutes>")
            sys.exit(1)
        ...
        while True:
            try:
                ...
                main(update_only=True, refresh_cache=refresh_cache)
                logger.info(f"Update complete. Waiting {interval_minutes} minutes...")
            except PermissionError as e:
                logger.warning("⚠️ Permission Error: Could not save the Excel file. Is it open? Please close it to allow updates.")
            except Exception as e:
                import traceback
        ...
        if '--gsheet' in sys.argv:
            try:
                ...
                logger.error("Usage: python main.py --gsheet <sheet_name_or_id>")
                sys.exit(1)
        
        main(gsheet_target=gsheet_target, refresh_cache=refresh_cache)
